...
contract SimpleNFTSale {
  ...
  constructor(address payable seller_, address payable buyer_, uint256 price_, IERC721 tokenContract_, uint256 tokenID_) {
    seller = seller_;
    buyer = buyer_;
    price = price_;
  ...
  function withdrawNFT(IERC721 tokenContractToWithdraw, uint256 tokenIDToWithdraw, address to) external {
    if (msg.sender != seller) {
      revert NotAuthorized();
    }
